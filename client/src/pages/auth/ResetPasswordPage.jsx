...
export const ResetPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (strength < 4) { setError('Password must be at least 8 characters with one uppercase letter, one number, and one special character.'); return; }
          ...
          <div className="p-6 md:p-8">
            {!success ? (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type={showPwd ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Min. 8 chars, uppercase, number, special" className="w-full bg-dark-800 border border-dark-600 rounded-xl pl-10 pr-10 py-3 text-sm text-white placeholder-dark-500 outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500/30 transition-all" disabled={loading || !token} />
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Lock className="w-4 h-4 text-dark-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
                      <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Re-enter your password" className="w-full bg-dark-800 border border-dark-600 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-dark-500 outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500/30 transition-all" disabled={loading || !token} />
                    </div>
                  ...
                  <button type="submit" disabled={loading || !token} className="w-full bg-teal-500 hover:bg-teal-400 text-dark-900 font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed mt-2">
              ...
              <div className="text-center py-4">
                ...
                <p className="text-dark-400 text-sm">Your password has been updated successfully. Redirecting you to sign in...</p>
              </div>
            )}
